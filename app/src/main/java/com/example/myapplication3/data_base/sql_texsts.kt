...
=======
    ...
    "       (21, 'Что делать, если попал в ДТП?', 'Базовая инструкция:\n" +
        "0. Сохраняйте спокойствие. ДТП может случиться с каждым, даже с самым добросовестным водителем, и в такой ситуации постарайтесь успокоиться, трезво собраться с мыслями и оставаться хладнокровными.\n" +
        ...
        "5. Ничего не трогайте и не перемещайте, хотя бы до того, как снимете на фото или видео место происшествия, если нужно убрать машины с дороги;\n" +
        ...
        "7. После фиксации всех фактов ДТП освободите дорогу, если мешаете другим машинам, и вызовите ГИБДД;\n" +
...
>>>>>>> 209ca33f70894cf1f4740ee6a598b24b7f1167c6
    ...
    "       (13, 'Я попал в ДТП с самокатом', '1. Для начала последуйте базовым указаниям по поведению в первые минуты ДТП;\n2. " +
        ...
        "       (1332, 'Я потерпевший, что делать?', '1. Следуйте базовым указаниям по поведению в первые минуты после ДТП;\n" +
        "2. Будьте открыты к предложению виновника договориться на месте, однако в таком случае постарайтесь рассчитать верно сумму, требующуюся Вам на ремонт;\n" +
        "3. Если всё же решились договориться на месте, составьте расписку об отсутствии претензий: Вы должны сойтись в том, каковы обстоятельства аварии, указать паспортные и контактные данные виновника и пострадавшего, номера машин, кто виноват, сколько он должен заплатить и когда. В расписке должны быть подписи как участников ДТП, так и 2 свидетелей;\n" +
        "4. Постарайтесь сразу же зафиксировать номер, марку и модель машины виновника, а также прочие детали по возможности. Если виновник попробует скрыться – сообщите об этом в ГИБДД. Вы всё равно сможете выдвинуть досудебную претензию виновнику или подать на него в суд.\n', 0),\n" +
        "       (14, 'Я попал в ДТП с пешеходом', '', 0),\n" +
        "       (16, 'Могу ли я уехать с места ДТП?', 'Вообще – нет, но есть несколько исключений, когда можно уехать с места ДТП.\n" +
        "*См. ст. 12.27 КоАП РФ\n', 0),\n" +
    ...
    "       (-24, 'Могу ли я участвовать в составлении протокола?', 'Конечно. Более того, Вы имеете право вписывать замечания и объяснения по содержанию протокола, что потом прилагается к нему.\n', 0),\n" +
    "       (-25, 'Что делать, если инспектор нарушает мои права?', 'У «инспектора» нет формы, нет значка или у него нестандартная форма? Вымогает у Вас взятку? Вы имеете право не подчиняться его требованиям. Срочно звоните по «телефону доверия» или в Управление собственной безопасности МВД. [Гиперссылка на контакты]. Изложите ситуацию специалистам максимально спокойно, чётко и конкретно, и ждите дальнейших указаний.', 0),\n" +
    "       (-26, 'Что делать, если инспектор отказался составлять протокол?', 'Если Вы считаете себя правым, то возразите: скажите, что Вы не нарушили ничего или что Вы не согласны со штрафом. Инспектор будет обязан выписать протокол.\n" +
        "*См. ч. 2 ст. 28.6 КоАП РФ\n', 0),\n" +
    ...
    "       (11222, 'Я пострадавший, что мне делать?', '1. Следуйте базовым указаниям по поведению в первые минуты после ДТП;\n" +
        "2. Будьте открыты к предложению виновника договориться на месте, однако в таком случае постарайтесь рассчитать верно сумму, требующуюся Вам на ремонт;\n" +
        "3. Если всё же решились договориться на месте, составьте расписку об отсутствии претензий: Вы должны сойтись в том, каковы обстоятельства аварии, указать паспортные и контактные данные виновника и пострадавшего, номера машин, кто виноват, сколько он должен заплатить и когда. В расписке должны быть подписи как участников ДТП, так и 2 свидетелей;\n" +
        "4. Постарайтесь сразу же зафиксировать номер, марку и модель машины виновника, а также прочие детали по возможности. Если виновник попробует скрыться – сообщите об этом в ГИБДД. Вы всё равно сможете выдвинуть досудебную претензию виновнику или подать на него в суд.\n', 0),\n" +
    ...
    "       (37, 'Насколько сильно можно тонировать стекла?', 'Зависит от того, какие именно Вы хотите затонировать.\n" +
        "Лобовое – только на 30%;\n" +
        "Передние боковые – только на 30%;\n" +
        "Задние боковые – без ограничений;\n" +
        "Заднее – без ограничений, если стоят наружные боковые зеркала заднего вида.\n" +
        "*См. п. 5.1.2.5. ГОСТа 32565-2013. Национальный стандарт Российской Федерации. Стекло безопасное для наземного транспорта. Общие технические условия, п. 4 ТР ТС 018/2011. Технический регламент Таможенного союза. О безопасности колесных транспортных средств.\n', 0),\n" +
    "       (371, 'Какие штрафы есть за тонировку стекла?', '500 рублей. И ещё Вас обяжут снять тонировку, если она слишком сильная.\n" +
        "*См. ч. 3.1. ст. 12.5 КоАП РФ\n', 0),\n" +
    "       (372, 'Как замеряется тонировка?', 'Вашу тонировку проверяют на светопропускаемость специальным прибором – тауметром. Тауметр должен проходить ежегодную проверку и иметь сертификат. Также на приборе должны быть клеймо с датой следующей проверки и личная пломба проверяющего. При проверке на дисплее прибора появляются цифры, которые указывают процент пропускания света.\n" +
        "Так что инспектор должен показать Вам зафиксированные показания тауметра. При этом Вы имеете право просмотреть сертификат на него, в котором содержится информация о проверке и погрешности прибора. Это может помочь Вам оспорить нарушение как по погрешности, так и по отсутствию пометки о проверке.\n" +
        "*См. ст. 26.8, ч. 1 ст. 25.1. КоАП РФ, ст. 151 Приказа МВД России от 23.08.2017 № 664 «Об утверждении Административного регламента по надзору за соблюдением требований безопасности дорожного движения»\n', 0),\n" +
    "       (373, 'Как обжаловать штраф за тонировку?', 'Здесь [ссылка внутри документа на раздел «Как обжаловать?»] Вы найдёте информацию о том, как подать жалобу, а также советы до и во время её подачи.', 0),\n" +
